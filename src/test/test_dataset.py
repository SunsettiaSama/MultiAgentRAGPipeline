...
def test_normal():
    ...
    forest = Forest(sampling_nums = 4, sampling_decay = 2)

    input_prompts = ["This is a test" for i in range(2)]
    query_lists = [copy.deepcopy(["This is a test"]) for i in range(2)]
    document_lists = [copy.deepcopy(["This is a test"]) for i in range(2)]
    time_lis = [0, 0]
    done_flags = [False, False]

    def pipeline(query_lists, *args, **kwargs):
        decision_prompts, time_lis, model_input_prompts = ["You Got This!" for i in range(len(query_lists))], [1 for i in range(len(query_lists))], ["Come on." for i in range(len(query_lists))]
        return decision_prompts, time_lis, model_input_prompts

    forest.update(done_flags, time_lis = time_lis, query_lists = query_lists, document_lists = document_lists)
    for i in range(3):
        # OK
        query_lists, document_lists, time_lis, done_flags = forest.augment_sampling(query_lists, document_lists, time_lis, done_flags)
        model_output_prompts, time_lis, model_input_prompts = pipeline(query_lists, document_lists, time_lis, done_flags)
        forest.update(model_inputs = model_input_prompts, model_outputs = model_output_prompts, time_lis = time_lis, done_flags = done_flags)
...
def test_complex_version():
    ...
    forest = Forest(sampling_nums = 8, sampling_decay = 2)

    input_prompts = ["This is a test" for i in range(2)]
    query_lists = [copy.deepcopy(["This is a test"]) for i in range(2)]
    document_lists = [copy.deepcopy(["This is a test"]) for i in range(2)]
    time_lis = [0, 0]
    done_flags = [False, False] # 此时不应该有任何形式的更新
...
def test_forest_reward_delivery():
    """测试通过"""
    from lib.dataset import Forest


    forest = Forest(sampling_nums = 8, sampling_decay = 2)
    dummy_excutor = lambda x: x
    ...
    model_input_prompts = [f"Model Input Prompts {str(i)}" for i in range(len(query_lists))]
    model_output_prompts = [f"Model Output Prompts {str(i)}" for i in range(len(query_lists))]
    time_lis = [i for i in range(len(query_lists))]

    # 进行动作后，修改第二个done
    query_lists = dummy_excutor(query_lists) 
    document_lists = dummy_excutor(document_lists) 
    done_flags = dummy_excutor(done_flags)
    time_lis = dummy_excutor(time_lis) 
    ...
    # 检查第二个done_flags是否为True，实际上这个是上一层，也即第1层的done_flags
    print("=" * 20)
    print("Checking 2nd Layer done flags...")
    print("Done Flags [Index 2]: ", done_flags[2] == True)
    print("Done Flags [Index 9]: ", done_flags[9] == True)
    print("=" * 10)
    ...
    ultimate_node_results = forest.get_end_nodes() # 测试通过！！！！！

    # 检查第二个done_flags是否为True，实际上这个是上一层，也即第1层的done_flags
    done_flags = [True for i in range(len(done_flags))]
    forest.update(model_inputs = model_input_prompts, model_outputs = model_output_prompts, time_lis = time_lis, done_flags = done_flags)
    ...
    final_model_input_prompts = [f"Final Model Input Prompts {str(i)}" for i in range(len(query_lists))]
    final_model_output_prompts = [f"Final Model Output Prompts {str(i)}" for i in range(len(query_lists))]
    final_time_lis = [i for i in range(len(query_lists))]

    forest.add_additional_layer(final_model_inputs = final_model_input_prompts, final_model_outputs = final_model_output_prompts, final_time_lis = final_time_lis)

    #####################
    # 外部奖励分析
    #####################
    # from lib.dataset import visualize_forest
    # visualize_forest(forest = forest)

    rewards = [i for i in range(len(query_lists))]
    time_lis = forest.get_time_costs()
    queries, responses, rewards = forest.delivery_rewards(rewards)

    # 测试通过
    root_indices = forest.get_root_indices_for_leaf_nodes()
    ultimate_node_results = forest.get_end_nodes()
    forest.clear()


def test_forest_delivery_rewards():
    ...
    model_input_prompts = [f"Model Input Prompts 1.{str(i)}" for i in range(len(query_lists))]
    model_output_prompts = [f"Model Output Prompts 1.{str(i)}" for i in range(len(query_lists))]
    time_lis = [i for i in range(len(query_lists))]
    ...
    model_input_prompts = [f"Model Input Prompts 2.{str(i)}" for i in range(len(query_lists))]
    model_output_prompts = [f"Model Output Prompts 2.{str(i)}" for i in range(len(query_lists))]
    time_lis = [i for i in range(len(query_lists))]
    ...
    model_input_prompts = [f"Model Input Prompts 3.{str(i)}" for i in range(len(query_lists))]
    model_output_prompts = [f"Model Output Prompts 3.{str(i)}" for i in range(len(query_lists))]
    time_lis = [i for i in range(len(query_lists))]
    forest.update(model_inputs = model_input_prompts, model_outputs = model_output_prompts, time_lis = time_lis, done_flags = done_flags)
    ...
    ultimate_node_results = forest.get_end_nodes() # 测试通过！！！！！
    for node in ultimate_node_results:
        print("=" * 20)
        print(node.layer_index)
        print(node.node_index)
        print(node.input)
        print(node.parent_index)
        print(node.children_index)
    ...
    # 第一轮一般奖励
    rewards = [i / len(query_lists) for i in range(len(query_lists))]
    rewards[0] = 0
    rewards[1] = 0
    rewards[2] = 1
    rewards[3] = 1
    ...
    # 第二轮极端样本
    rewards = [0 for i in range(len(query_lists))]
    queries, responses, rewards = forest.delivery_rewards(rewards)

    if len(queries) == len(responses) and len(queries) == len(rewards):
        print("=" * 20)
        print("1. Done")
        print(len(queries))

    # 第三轮极端样本
    rewards = [1 for i in range(len(query_lists))]
    queries, responses, rewards = forest.delivery_rewards(rewards)
    if len(queries) == len(responses) and len(queries) == len(rewards):
        print("=" * 20)
        print("2. Done")
